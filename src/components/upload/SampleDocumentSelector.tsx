...
const SampleDocumentSelector = ({ handleSampleDocument }: { handleSampleDocument: (doc: any) => void }) => {
    return (
        <div className="mb-6">
            <h3 className="text-lg font-semibold text-gray-700 mb-3 dark:text-gray-200">Try Sample ACORD Documents:</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {SAMPLE_DOCUMENTS.map((doc, idx) => (
                    <button
                        key={idx}
                        onClick={() => handleSampleDocument(doc)}
                        className="p-4 border border-gray-200 rounded-lg hover:border-indigo-400 dark:hover:border-indigo-400 hover:bg-indigo-50 dark:bg-input/30 dark:border-input dark:hover:bg-input/50 transition-all text-left"
                    >
                        <FileText className="w-5 h-5 text-indigo-600 mb-2 dark:text-indigo-400" />
                        <div className="font-medium text-gray-800 text-sm dark:text-gray-200">{doc.name}</div>
